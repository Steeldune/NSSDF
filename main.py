...
class Wiener():
    def __init__(self, wiener_sample, scale=None, start=None,
                 ...
                 K=1.0):  # Accepts an array with two rows, one with a Wiener process, the other the time samples.
        if scale is None:
    ...
    @classmethod
    def generate_new(cls, scale, end, start=0.0, h=1.0, K=1.0):
        ...
        return cls(np.array([axis_x, sample]), scale=scale, start=start, end=end, h=h, K=K)
    ...
    @classmethod
    def generate_downscale(cls, wiener, h=1.0, K=1.0):
        ...
        return cls(downscale_wiener, h=h, K=K)
...
if __name__ == '__main__':
    '''These are the parameters by which the code can be altered, after the E_or_M parameter, 
    all code should function on its own'''
    depth = 1.0  # This is h, the scale of the simulation
    diffusion = 1.0  # This is K, the speed of the diffusion
    nr_samples = 1000  # The number of particles that will be simulated
    time_end = 1  # t starts at 0, and ends at time_end
    time_scale = 0.001  # This is dt, the time-scale of the most accurate simulation
    nr_scales = 7  # This is the number of additional particles will be simulated at lower sampling of the Wiener
    # process. Each subsequent simulation will double the time-scale.
    ini_point = depth / 4  # Where the particles start in z
    nr_histo_frames = 5  # The number of histogram frames will be rendered of the particle distribution. They will be
    # made with regular intervals of time in between.
    E_or_M = False  # Is the simulation running the Euler (True) or Milstein scheme (False)

    scale_origin = 0
    nr_solvers = nr_scales
    mfd = int(2 ** (nr_scales - 1))
    rng = default_rng()

    wiener_list = [None for x in range(nr_scales)]
    wiener_list[scale_origin] = [Wiener.generate_new(time_scale, time_end, h=depth, K=diffusion) for i in
                                 range(nr_samples)]
    ...
    for i in range(scale_origin + 1, nr_scales):
        wiener_list[i] = [Wiener.generate_downscale(wiener.export(), h=depth, K=diffusion) for wiener in
                          wiener_list[i - 1]]
    ...
    for j in tqdm(range(nr_samples)):
        ...
        for i in range(nr_scales - nr_solvers, nr_scales):
            if E_or_M:
                ...
                solved_list[i][j] = wiener_list[i][j].apply_milstein(f_func, g_func, g_func_der,
                                                                     ini_pos=ini_point)
